...
public class DependencyArchitectureTests
{
    ...
    public void Should_not_depend_on_application_layers()
    {
        var result = Types.InAssembly(ArchitectureTestAssembly.ProductionAssembly)
            .That().ResideInNamespace("Norge360.Persistence.EntityFrameworkCore.SoftDelete")
            .ShouldNot().HaveDependencyOnAny(DisallowedDependencies)
            .GetResult();

        ArchitectureAssertions.AssertRule(result, "Norge360.Persistence.EntityFrameworkCore.SoftDelete should not depend on application-specific layers.");
    }
}
// <copyright file="HttpDiscoveryEventPublisher.cs" company="Norge360">
    ...
    ILogger<HttpDiscoveryEventPublisher> logger) : IDiscoveryEventPublisher
{
    ...
    {
        var client = httpClientFactory.CreateClient("discovery-accounts");
    ...
    private void AddInternalTokenHeader(HttpRequestMessage request)
    {
        ...
        {
            logger.LogWarning("Discovery internal token is not configured for Accounts publisher.");
            return;
        }

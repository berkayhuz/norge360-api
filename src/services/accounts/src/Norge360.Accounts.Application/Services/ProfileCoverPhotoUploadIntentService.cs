// <copyright file="ProfileCoverPhotoUploadIntentService.cs" company="Norge360">
...
public sealed class ProfileCoverPhotoUploadIntentService(
    IMediaUploadUrlSigner uploadUrlSigner,
    IMediaUrlBuilder mediaUrlBuilder,
    IOptions<MediaOptions> mediaOptions) : IProfileCoverPhotoUploadIntentService
{
    private const long MaxCoverPhotoBytes = 2L * 1024 * 1024;
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (errors.Count > 0)
        {
            return Task.FromResult(
                CreateAvatarUploadIntentResult.ValidationFailed(
                    ...
                    "cover_photo_upload_intent_validation_failed"));
        }
        ...
        var storageKey =
            $"profiles/{authUserId.ToString("D").ToLowerInvariant()}/cover-photo/{now:yyyy}/{now:MM}/{Guid.NewGuid():N}{extension}";
        ...
        {
            return Task.FromResult(CreateAvatarUploadIntentResult.Failed("cover_photo_upload_intent_not_supported"));
        }
        catch
        {
            return Task.FromResult(CreateAvatarUploadIntentResult.Failed("cover_photo_upload_intent_failed"));
        }
    }
    ...
    private static Dictionary<string, List<string>> Validate(CreateAvatarUploadIntentRequest request, MediaOptions options)
    {
        ...
        {
            if (request.ContentLength > MaxCoverPhotoBytes)
            {

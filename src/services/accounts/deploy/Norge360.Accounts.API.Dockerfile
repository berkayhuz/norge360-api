...
RUN dotnet restore src/services/accounts/src/Norge360.Accounts.API/Norge360.Accounts.API.csproj --force-evaluate
RUN dotnet publish src/services/accounts/src/Norge360.Accounts.API/Norge360.Accounts.API.csproj -c Release --no-restore -o /app/publish -p:UseAppHost=false

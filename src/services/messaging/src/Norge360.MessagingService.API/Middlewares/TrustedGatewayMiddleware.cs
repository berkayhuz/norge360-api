...
namespace Norge360.MessagingService.API.Middlewares;
    ...
    ILogger<TrustedGatewayMiddleware> logger)
{
    public async Task InvokeAsync(HttpContext context)
    {
        ...
        if (!trustedGatewayOptions.RequireTrustedGateway ||
            allowAnonymous ||
        ...
        {
            logger.LogWarning(
                "Trusted gateway validation rejected messaging request for {Path}. Reason={Reason} CorrelationId={CorrelationId}",
            ...
            await context.Response.WriteAsJsonAsync(
                new
                {
                    ...
                    detail = "The messaging service only accepts this endpoint via a trusted gateway.",

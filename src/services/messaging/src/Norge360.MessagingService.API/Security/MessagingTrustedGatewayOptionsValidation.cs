// <copyright file="MessagingTrustedGatewayOptionsValidation.cs" company="Norge360">
...
namespace Norge360.MessagingService.API.Security;

public sealed class MessagingTrustedGatewayOptionsValidation(IHostEnvironment environment) : IValidateOptions<TrustedGatewayOptions>
{
    public ValidateOptionsResult Validate(string? name, TrustedGatewayOptions options)
    {
        ...
        if (environment.IsProduction() &&
            options.AllowedGatewayProxies.Length == 0 &&
            options.AllowedGatewayNetworks.Length == 0)
        {
